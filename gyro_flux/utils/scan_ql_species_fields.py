...
def scan_ql_species_fields(base_dir: str = "/home/shared_info/Well_Formatted_CGYRO_W_TGLF"):
    ...
    # Get all subdirectories, including folders starting with "2020_04-Marinoni_QH"
    # and not d.name.startswith("2020_04-Marinoni_QH")
    subdirs = [
...
def scan_marinoni_folders(base_dir: str = "/home/shared_info/Well_Formatted_CGYRO_W_TGLF"):
    base_path = Path(base_dir)
    ...
    # Get only subdirectories starting with "2020_04-Marinoni"
    subdirs = [
        d for d in base_path.iterdir() 
        if d.is_dir() and d.name.startswith("2020_04-Marinoni")
    ]
    ...
    # Print results
    print(f"Total yaml files scanned (2020_04-Marinoni folders): {total_scanned}")
    print()
    print("n_species, n_fields:")
    print("-" * 40)
